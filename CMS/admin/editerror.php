...
if (isset($_POST['contents']))
{
	...
	$query = "Update `settings` Set value = '$contents' where name = '404error' and siteId = \"$site_id\"";
	$result = mysql_query($query) or die ('Putting data into database failed!');

	$query = "Update `settings` Set value = '$name2' where name = '404error_title' and siteId = \"$site_id\"";
	$result = mysql_query($query) or die ('Putting data into database failed!');

	echo '<font class="c595">Edit Error Page</font><br />Click <a href="editerror.php">Here</a> to continue editing.';
	echo '<script type="text/javascript">window.top.location="editerror.php";</script>';
}
else
{
	$edit = $_REQUEST['edit'];

	$query = "Select value from `settings` where name = '404error' and siteId = \"$site_id\"";
	$result = mysql_query($query) or die ('Getting data from database failed!');
	$contents = mysql_result($result, 0, "value");

	$query = "Select value from `settings` where name = '404error_title' and siteId = \"$site_id\"";
	$result = mysql_query($query) or die ('Getting data from database failed!');
	$name = mysql_result($result, 0, "value");

	echo '<font class="c595">Edit Error Page</font><br /><a href="stat.php?id=404error">View Stats</a><br />';
	...
	echo '<form method="post" action="editerror.php">
		Title: <input type="text" value="' . $name . '" name="name"><br />
		<input type="submit" value="Save" onclick="window.saving=\'true\';"><br /><br />
		<textarea name="contents" rows="15" style="width:100%">' . $contents . '</textarea>
